...
func (sk *StreamKey) Put(k ExtentKey) {
	...
	if lastKey.PartitionId == k.PartitionId && lastKey.ExtentId == k.ExtentId && lastKey.ExtentOffset==k.ExtentOffset {
	...
	for i := 0; i < extentsLen; i++ {
		ek := sk.Extents[i]
		if ek.PartitionId == k.PartitionId && ek.ExtentId == k.ExtentId && ek.ExtentOffset==k.ExtentOffset {
			if k.Size > ek.Size {

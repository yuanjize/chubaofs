...
func (s *ExtentStore) RepairRead(extentId uint64, offset, size int64, nbuf []byte) (crc uint32, err error) {
	var extent *Extent
	if extent, err = s.getExtentWithHeader(extentId); err != nil {
		return
	}
	...
	crc, err = extent.RepairRead(nbuf, offset, size)
	return
}
...
func (s *ExtentStore) initTinyExtent() (err error) {
	...
	for extentId = TinyExtentStartId; extentId < TinyExtentStartId+TinyExtentCount; extentId++ {
		...
		if err != nil && !strings.Contains(err.Error(), syscall.EEXIST.Error()) && err != ErrorExtentHasExsit {
			return
		}
		err = nil
